  ...
  describe("searchChannel", () => {
    ...
    TEST_CHANNELS.forEach(({ id, name }, index) => {
      it(`common query ${index + 1}`, async () => {
        const res = await chzzkClient.searchChannel({
          query: name,
          size: 5,
        });
        ...
        expect(res.content.data[0].channel.channelId).toBe(id);
      });
    });

    it("empty query", async () => {
      await expect(() =>
        chzzkClient.searchChannel({ query: "" })
      ).rejects.toThrowError("400");
    });

    it("pagination", async () => {
      const res1 = await chzzkClient.searchChannel({
        query: "치지직",
        size: 5,
      });
      ...
      const res2 = await chzzkClient.searchChannel({
        query: "치지직",
        size: 5,
        next,
      });
